...
bool gameOver(){
    if((posRocket>=0.6 && posRocket<=1 && position1<-1.8) || (posRocket>= -1 && posRocket<=-0.6 && position<=-1.8 ) || (posRocket>=-0.3 && posRocket<=0.3 && position2<=-1.8))return 1;
    return 0;
}
void gameScore(){
    ...
    else if(posRocket>= -1 && posRocket<=-0.6 && ammoPos-1.7>=position && ammoPos!=0 ){
        cout<<"Enemy killed! Point - "<<++gamePoint<<endl;
        ammoPos=0.0f;
        position=1.2f;
    }
    else if(posRocket>=-0.2 && posRocket<=0.2 && ammoPos-1.7>=position2 && ammoPos!=0){
        cout<<"Enemy killed! Point - "<<++gamePoint<<endl;
        ammoPos=0.0f;
        position2=1.2f;
    }
